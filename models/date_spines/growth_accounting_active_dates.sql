...
{% set end_date = dbt_utils.get_query_results_as_dict('select max(active_at)::varchar end_date from ' ~ ref('growth_accounting_daily_activity__stg')) %}
{% set start_date = dbt_utils.get_query_results_as_dict('select min(active_at)::varchar start_date from ' ~ ref('growth_accounting_daily_activity__stg')) %}
